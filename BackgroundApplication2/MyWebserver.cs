    ...
    internal class MyWebserver
    {
        ...
        public async void Start()
        {
            ...
            listener.ConnectionReceived += async (sender, args) =>
            {
                ...
                if (path.Contains("ON") || path.Contains("OFF"))
                {
                    new IotController().ProcessRemoteOrder(path);
                }
                ...
                using (var output = args.Socket.OutputStream)
                {
                    using (var response = output.AsStreamForWrite())
                    {
                        ...
                        $"<html><head><title>Background Message</title></head><body>Hello from the background process!<br/>{path}</body></html>");
        ...
        private static string GetPath(StringBuilder request)
        {
